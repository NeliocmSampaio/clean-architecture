...
from usecases.user.find_user.find_user_dto import FindUserInputDto, FindUserOutputDto, TaskOutputDto
...
class FindUserUseCase(UseCaseInterface):
    ...
    def execute(self, input: FindUserInputDto) -> FindUserOutputDto:
        ...
        tasks = self.task_repository.list_tasks_from_user(user_id=user.id)

        user.collect_tasks(tasks=tasks)
